...
### M1 — 1채널 재생 코어
- **선행 조건: 테스트 장비에 voxl-streamer-watchdog(v3 passive) 설치** — 2026-06-12 장비 공장초기화로 제거된 상태이므로 `viewer/watchdog/`에서 재배포 필요 (직결 소크에서 viewer 재시작·링크 단절이 인코더 재오픈 경로를 밟으므로, watchdog 부재 시 소크 실패 원인에 환경 미비가 섞인다. M1 소크는 watchdog의 실전 검증 기회를 겸한다. 프로비저닝 문서화 자체는 M5)
