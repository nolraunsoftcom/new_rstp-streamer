...
`DeviceUnreachable, RelayDown, RelayNoSource, SessionRefused, NoPackets, DecodeError, DiskLow, DiskFull, ...` — UI 문구·로그·테스트가 같은 코드를 공유. 모호한 에러 메시지 원천 차단. `SystemResourceMonitor`는 CPU·메모리에 더해 **녹화 대상 디스크 잔량을 감시**하고, 임계 미달 시 `DiskLow`(경고)/`DiskFull`(녹화 중단 사유)를 발생시켜 상태바·진단 패널에 표시한다 — 장기 무인 가동에서 가장 흔한 녹화 실패 원인의 사전 가시화.
...
**세그먼트 정책**: 녹화 중 demux 재연결이 발생하면 타임스탬프 불연속이 생기므로 기존 파일을 닫고 새 세그먼트로 시작한다. 장시간 녹화의 주기적 분할(예: 10분 단위 롤오버)도 같은 메커니즘으로 처리한다.
...
### M1 — 1채널 재생 코어
- **선행 조건: 테스트 장비에 voxl-streamer-watchdog 설치** (직결 소크에서 viewer 재시작·링크 단절이 인코더 재오픈 경로를 밟으므로, watchdog 부재 시 소크 실패 원인에 환경 미비가 섞인다. M1 소크는 watchdog의 실전 검증 기회를 겸한다. 프로비저닝 문서화 자체는 M5)
