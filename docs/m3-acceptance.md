...
## 실기기 테스트 배터리 결과 (2026-06-13, VOXL2 Mini H.264 640x480)

### Phase 1 — 1시간 스트리밍 + 연속 녹화 + 20초 캡처 (--auto-record --snapshot-every=20)
- **녹화 세그먼트 7개 전부 재생 가능**(손상 0): 421초(재연결 분리) + 600초×5(maxDuration 롤오버) + 154초(꼬리). 롤오버·재연결 분리 모두 작동, 1시간 내내 녹화 생존.
- **메모리 누수 없음**: RSS 200→204MB(58분, 116샘플), 최대 234MB — 평탄.
- **캡처 179장, 손상 0**.
- 끊김 1~2회(장비측, 자동 복구), 거짓종료 경고 0, 디스크오류 0, 크래시 0, wedge 0, 백오프중단 0.

### Phase 2 — 녹화 토글 스트레스 15분 (--record-toggle=8, 112회 토글)
- **세그먼트 56개 전부 재생 가능(손상 0)**, 크래시 0, 좀비 녹화 0, 백오프중단 0.

### 검증된 요구사항
- 1시간 영상 스트리밍 안정성 ✅ (RSS 평탄, 끊김 자동복구)
- 캡처 스트레스 ✅ (179장 무손상)
- 녹화 스트레스 ✅ (56 토글 세그먼트 무손상)
- 캡처/녹화 중 연결 안정성 ✅ (장비 끊김 통과하며 녹화 생존, 전 세그먼트 재생가능)

**판정: M3 실기기 안정성 PASS — 추가 리팩토링 불요.**
