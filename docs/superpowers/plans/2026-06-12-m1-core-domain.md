...
3. **`Failed`는 종착역.** `tick`으로는 절대 빠져나오지 않고 수동 `retryRequested`만 가능.
...
struct ReconnectPolicy {
    int maxAttempts = 30;                              // 초과 시 Failed (give-up)
    std::chrono::milliseconds retryDelay{5000};        // 재시도 간격 (고정)
};
...
Transition ConnectionStateMachine::tick(TimePoint now) {
    switch (m_state) {
        ...
        case ConnState::Stalled:
            ...
            return {};
        default:
            return {};  // Idle, Connecting, Failed: tick으로 변하지 않음
    }
}
...
TEST_CASE("Failed는 tick으로 절대 벗어나지 않는다") {
    auto sm = makeSm();
    sm.connectRequested(t0);
    auto now = t0;
    while (sm.state() != ConnState::Failed) {   // give-up까지 진행
...
TEST_CASE("Failed에서 retryRequested만이 새 사이클을 연다") {
    auto sm = makeSm();
    sm.connectRequested(t0);
    auto now = t0;
    ...
    CHECK(sm.state() == ConnState::Connecting);
    CHECK(t.action == Action::OpenSource);
    CHECK(sm.reconnectAttempts() == 0);   // 사용자 명령 = 새 사이클
}
```
...
// 진단 6단계(StreamHealth)를 유지한다. UI 스레드에서만 사용 (설계 D6).
...
1. `ctest` 전체 통과 — 상태머신 전이(정상/가짜연결/stall/give-up/수동복구/경계) 전수 검증
