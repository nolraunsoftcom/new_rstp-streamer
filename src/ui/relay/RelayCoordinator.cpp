...
void RelayCoordinator::poll() {
    // 워커 스레드의 타이머 콜백. pollHealth(QNAM)/restart 등에서 예외가 새면 std::terminate
    // (0xc0000409)로 앱이 죽으므로 경계에서 잡는다.
    try {
        ...
        std::fprintf(stderr, "[RelayCoordinator::poll] threw: %s\n", e.what());
    } catch (...) {
        std::fprintf(stderr, "[RelayCoordinator::poll] threw (unknown)\n");
    }
}
...
void RelayCoordinator::updateChannels(std::vector<nv::domain::RelayPath> channels) {
    ...
    try {
        ...
        std::fprintf(stderr, "[RelayCoordinator::updateChannels] threw: %s\n", e.what());
    } catch (...) {
        std::fprintf(stderr, "[RelayCoordinator::updateChannels] threw (unknown)\n");
    }
}
