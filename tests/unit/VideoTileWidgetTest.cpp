...
// (offscreen QPA에서도 top-level show 후 visibleRegion이 위젯 영역으로 채워진다.)
void makeVisible(nv::ui::VideoTileWidget& tile) {
    tile.resize(64, 64);
    tile.show();
...
// D9: 보이지 않는(숨김) 타일은 pollFrame에서 latestSurface 조회·복사를 스킵한다.
TEST_CASE("VideoTileWidget: 숨김 타일은 pollFrame이 서피스를 폴링하지 않는다 (D9)") {
    (void)getApp();
    FakeRegistry reg;
    void* fakeHandle = reinterpret_cast<void*>(0xFEEDFACE);
    reg.setGpuSurface(fakeHandle, /*seq=*/1);

    nv::ui::VideoTileWidget tile(reg, "ch1");
    // show하지 않음 → isVisible()==false → 폴링 스킵.
    tile.pollFrame();

    REQUIRE(reg.releaseConsumedCallCount == 0);   // 조회·반납 모두 없음(스킵됨)
}
