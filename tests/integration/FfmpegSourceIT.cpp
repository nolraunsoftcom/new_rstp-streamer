...
// HW(VideoToolbox) 또는 SW 폴백 — 어느 경로든 슬롯에 서피스(kind != None)가 도달해야 한다.
// macOS에서 H.264는 보통 VideoToolbox(GpuTexture)로, 미지원 환경은 CpuRgba로 폴백된다.
// 어느 쪽이든 표시 가능한 서피스가 나오는 것이 계약.
TEST_CASE("IT: H.264 — HW/SW 어느 경로든 서피스(kind!=None) 슬롯 도달") {
    if (!integrationEnabled()) SKIP("NV_MEDIAMTX_BIN 미설정");
    Server server;
    ChildProcess pub(publishCmd("h264.mkv", "thw"));
    std::this_thread::sleep_for(800ms);

    nv::infra::LatestSurfaceSlot slot;
    nv::infra::FfmpegStreamSource src(slot);
    WaitingListener lsn;
    src.open(playUrl("thw"), lsn);

    CHECK(lsn.waitFor("decoded", 15s));
    nv::app::FrameSurface surf;
    bool got = false;
    for (int i = 0; i < 100 && !got; ++i) {
        got = slot.latest(surf, 0);
        std::this_thread::sleep_for(100ms);
    }
    REQUIRE(got);
    CHECK(surf.kind != nv::app::FrameSurface::Kind::None);
    CHECK(surf.width == 640);
    CHECK(surf.height == 480);
    // HW 경로면 GpuTexture(동반 RGBA 포함), SW 폴백이면 CpuRgba — 어느 쪽이든 RGBA가 차 있어 표시 가능.
    CHECK(surf.rgba.size() == static_cast<size_t>(640) * 480 * 4);
    // GpuTexture로 넘어온 핸들은 슬롯이 추가 retain해 준 소비자 ref — 여기서 release(소비 종료).
    if (surf.kind == nv::app::FrameSurface::Kind::GpuTexture && surf.gpuHandle != nullptr) {
        slot.releaseConsumed(surf.gpuHandle);
    }
    src.close();
}
